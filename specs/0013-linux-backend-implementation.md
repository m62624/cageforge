# Specification 0013: Linux Backend Implementation
...
- proxy, MITM, SOCKS5, and managed-network product infrastructure; and
...
| proxy-only network flag | Managed Codex proxy routing | Excluded until a separate network runtime exists |
| proxy route specification | Internal TCP/UDS bridge setup | Excluded from Linux backend public API |
...
| system versus bundled bwrap | Launcher source | First version uses validated system bwrap; bundling is a later licensed component |
...
| glob scan depth | Bound for deny-glob expansion | Rejected with a typed unsupported-capability error until a native expansion implementation is added |
...
### 4.3 Planned Cageforge public API

The first Linux crate must expose a typed construction and execution surface
equivalent to:

```text
LinuxBackendConfig
  ├── bubblewrap source: discover system or explicit validated path
  ├── proc mount policy: required or explicitly disabled
  └── process-hardening policy: required by default
...
The exact Rust names may change during implementation, but the ownership
rules are fixed:
...
| deny globs and scan depth | Reject with typed `UnsupportedCapability`; never silently ignore a glob or fall back to a broader filesystem view |
...
| domain rules | Return typed unsupported until a resolver and exact-connect runtime are implemented |
| private/loopback/link-local restrictions | Support only through exact runtime network enforcement; otherwise return typed unsupported |
| `ResolvedNetworkTarget` authorization | Require one captured target and immediate exact-address authorization; never use hostname-only approval |
| Unix socket rules | Return typed unsupported until Linux-specific AF_UNIX enforcement is implemented |
...
the fresh PID namespace, and the authenticated helper runtime under
`/dev/shm/cageforge`. A policy rule that targets one of these runtime paths or
a descendant is rejected during lowering rather than being silently shadowed
by internal mounts. The helper mount is read-only and the containing
`/dev/shm` tmpfs is remounted read-only before the user command runs. A
root-wide policy necessarily includes the reserved helper path, but the
helper itself is not writable by the child.
...
- deny globs and their scan-depth limit are rejected before lowering because
  this backend has no native expansion stage yet;
- read-only subpaths;
...
Network namespace isolation can implement all-network-disabled behavior, but
it does not by itself implement domain allowlists or exact resolved-address
authorization.

The first backend may advertise network-disabled and unrestricted-network
capabilities only when those modes are actually enforced. It must not advertise
`NetworkDomainRules` or `NetworkResolvedTargets` until a Linux network runtime
can:
...
If the backend cannot provide that flow, a domain-restricted request must fail
with a typed unsupported-capability error. It must never be widened to network
enabled or hostname-only authorization.
...
The first crate should remain one public backend crate, with focused private
modules rather than several crates created only to split files:

```text
crates/cageforge-linux/
├── src/
│   ├── lib.rs          # public backend entry points and Linux cfg boundary
│   ├── backend.rs      # SandboxBackend implementation
│   ├── lowering.rs     # EffectiveSandbox to Linux plan
│   ├── bwrap.rs        # executable discovery and argument plan
│   ├── hardening.rs    # no_new_privs and seccomp
│   ├── process.rs      # launch, stdio, timeout, signals, cleanup
│   └── error.rs        # typed Linux backend errors
└── tests/
    ├── linux_filesystem.rs
    ├── linux_process.rs
    └── linux_network.rs
...
- an address outside the captured `ResolvedNetworkTarget` is rejected; and
- domain-restricted requests return typed unsupported errors until exact
  resolved-address enforcement exists.
...
1. install `bubblewrap`, `pkg-config`, and any native build packages required
   by the selected Rust dependencies;
2. enable unprivileged user namespaces and verify that the setting took
   effect;
3. build the backend and any runtime helper before running tests;
