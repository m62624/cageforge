...
| `EnvironmentSpec` and `EnvironmentNameKey` | `inherit_all`, `inherit_core`, `empty`, `base`, `overrides`, `override_for`, `filters`, `filter_action_for`, `with_var`, `without_var`, `with_filter`, `with_include_pattern`, `with_exclude_pattern`, `apply_to`; `EnvironmentNameKey::new` | Exercised by environment and trait-contract tests; maps to Codex inherited/empty environment construction, filtering, and set/remove overrides without importing Codex filtering or telemetry rules. Wildcard matching delegates to the same `wildmatch` crate used by Codex. | Keep. Variable names and filter patterns use one case-insensitive logical namespace without lossy native-string collisions. Patterns are compiled once, malformed native names fail closed when filtering is active, and the backend selects the platform-specific `Core` base map. |
