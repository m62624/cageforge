...
| `NetworkPolicy` | `disabled`, `enabled`, `unrestricted`, `external`, `mode`, `domain_mode`, `unix_socket_mode`, `local_network_access`, `with_domain_mode`, `with_unix_socket_mode`, `with_local_network_access`, `domains`, `unix_sockets`, `with_domain`, `with_unix_socket`, `validate`, `access_for_domain`, `decision_for_domain`, `decision_for_domain_with_resolved_ips`, `decision_for_resolved_target`, `decision_for_connected_address`, `decision_for_unix_socket`, `allows_unix_socket` | Exercised by network tests; separates network enforcement ownership and resolved-address safety from proxy/runtime concerns. | Keep. `enabled` retains the safe local-destination default; `unrestricted` is the explicit no-local-restriction preset. `access_for_domain` reports rule matching; the `decision_for_*` methods apply the complete policy and preserve `Allow`, `Deny`, and `ExternallyEnforced`. Resolved hostname checks fail closed on empty DNS results and non-public addresses, including allowlisted hostnames; `ResolvedNetworkTarget` additionally binds the decision to the exact socket address used by the backend. DNS remains a backend responsibility. The hostname-only decision is intentionally not exposed as a boolean authorization helper. Domain inputs normalize host case, ports, trailing dots, bracketed IP literals, and `globset`-compatible `*`, `?`, classes, and ranges. |
...
| `AuthorizedSocketAddr` and `ConnectionAuthorization` | `socket_addr`, `Allowed`, `Denied`, `ExternallyEnforced` | Exercised by resolved-target integration and composition property tests. | Keep. Only the exact address checked against a `ResolvedNetworkTarget` is returned as an allowed typed value; hostname-only inspection cannot produce this value. |
| `SandboxPolicy` | `new`, `read_only`, `workspace`, `full_access`, `filesystem`, `network`, `validate`, `normalized` | Exercised by built-in policy tests; combines the portable filesystem and network boundaries without exposing Codex `PermissionProfile` or legacy mode names. | Keep. `normalized` is the backend-ready form that collapses duplicate filesystem targets conservatively. This is the composition root for future profile resolution and backend preparation. |
...
| `EnvironmentSpec` | `inherit_all`, `empty`, `base`, `overrides`, `override_for`, `filters`, `filter_action_for`, `with_var`, `without_var`, `with_filter`, `with_include_pattern`, `with_exclude_pattern`, `apply_to` | Exercised by environment and trait-contract tests; maps to Codex inherited/empty environment construction, filtering, and set/remove overrides without importing Codex filtering or telemetry rules. Wildcard matching delegates to the same `wildmatch` crate used by Codex. | Keep. Variable names and filter patterns use one case-insensitive logical namespace, including `EnvironmentPattern` equality, hashing, and ordering, and the backend selects the platform-specific `Core` base map. |
