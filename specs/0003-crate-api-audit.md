...
| `NetworkPolicy` | `disabled`, `enabled`, `external`, `mode`, `domain_mode`, `unix_socket_mode`, `local_network_access`, `with_domain_mode`, `with_unix_socket_mode`, `with_local_network_access`, `domains`, `unix_sockets`, `with_domain`, `with_unix_socket`, `validate`, `access_for_domain`, `decision_for_domain`, `decision_for_domain_with_resolved_ips`, `decision_for_unix_socket`, `allows_domain`, `allows_unix_socket` | Exercised by network tests; separates network enforcement ownership from domain/socket defaults and resolved-address safety, as Codex does across protocol and native backends. | Keep. `access_for_domain` reports rule matching; the `decision_for_*` methods apply the complete policy and preserve `Allow`, `Deny`, and `ExternallyEnforced`. Resolved hostname checks fail closed on empty DNS results and non-public addresses by default; DNS remains a backend responsibility. Boolean helpers return true only for local `Allow`. Domain inputs normalize host case, ports, trailing dots, bracketed IP literals, and portable `*`/`?` globs. |
