...
fn resolves_on_linux(path: &Path) -> bool {
...
#[test]
fn every_configuration_example_is_checked_by_the_linux_crate() {
    ...
    for path in examples {
        ...
        if !resolves_on_linux(&path) {
            continue;
        }
        config
            .resolve_default_for_platform(PlatformId::Linux)
            .unwrap_or_else(|error| panic!("{} should resolve on Linux: {error}", path.display()));
        for profile in config.profile_names() {
            config
                .resolve_for_platform(profile, PlatformId::Linux)
                .unwrap_or_else(|error| {
                    panic!(
                        "{} profile {profile:?} should resolve on Linux: {error}",
                        path.display()
                    )
                });
        }
...
#[test]
fn local_ipc_example_selects_a_linux_unix_socket() {
    ...
    let resolved = Config::from_file(&path)
        ...
        .resolve_default_for_platform(PlatformId::Linux)
        .expect("Linux local IPC profile");
    let endpoint = resolved
        .policy()
        .network()
        .local_ipc()
        .first()
        .expect("Linux local IPC endpoint");
