...
fn resolves_on_windows(path: &Path) -> bool {
    let path = path.to_string_lossy();
    !path.contains("platform-targets-unix")
        && !path.contains("runnable/linux")
        && !path.contains("runnable/macos")
}

#[test]
fn every_configuration_example_is_checked_by_the_windows_crate() {
    ...
    for path in examples {
        ...
        if !resolves_on_windows(&path) {
            continue;
        }
        config
            .resolve_default_for_platform(PlatformId::Windows)
            .unwrap_or_else(|error| {
                panic!("{} should resolve on Windows: {error}", path.display())
            });
        for profile in config.profile_names() {
            config
                .resolve_for_platform(profile, PlatformId::Windows)
                .unwrap_or_else(|error| {
                    panic!(
                        "{} profile {profile:?} should resolve on Windows: {error}",
                        path.display()
                    )
                });
        }
...
#[test]
fn local_ipc_example_selects_a_windows_named_pipe() {
    ...
    let resolved = Config::from_file(&path)
        ...
        .resolve_default_for_platform(PlatformId::Windows)
        .expect("Windows local IPC profile");
    let endpoint = resolved
        .policy()
        .network()
        .local_ipc()
        .first()
        .expect("Windows local IPC endpoint");
    assert!(endpoint.endpoint().named_pipe().is_some());
}
