...
class SandboxProcess internal constructor(private var handle: Long) : Closeable {
    ...
    val stdout: InputStream? = if (hasStdout()) stream(true) else null

    /** A readable pipe when TOML configured stderr as `pipe`, otherwise null. */
    val stderr: InputStream? = if (hasStderr()) stream(false) else null

    /** A writable pipe when TOML configured stdin as `pipe`, otherwise null. */
    val stdin: OutputStream? = if (hasStdin()) {
        object : OutputStream() {
    ...
    fun tryWait(): ProcessResult? = synchronized(lifecycleLock) {
        checkOpen()
    ...
    fun kill() = synchronized(lifecycleLock) {
        checkOpen()
    ...
    private fun stream(stdout: Boolean): InputStream = object : InputStream() {
        ...
        private fun readBytes(size: Int): ByteArray {
            return synchronized(lifecycleLock) {
                checkOpen()
                if (stdout) NativeBridge.nativeReadStdout(handle, size)
                else NativeBridge.nativeReadStderr(handle, size)
            }
        }
    }

    private fun hasStdin(): Boolean = synchronized(lifecycleLock) {
        checkOpen()
        NativeBridge.nativeHasStdin(handle)
    }

    private fun hasStdout(): Boolean = synchronized(lifecycleLock) {
        checkOpen()
        NativeBridge.nativeHasStdout(handle)
    }

    private fun hasStderr(): Boolean = synchronized(lifecycleLock) {
        checkOpen()
        NativeBridge.nativeHasStderr(handle)
    }

    private fun decode(status: Int): ProcessResult? = when {
        status == -1 -> null
    ...
    internal companion object {
        fun fromHandle(handle: Long): SandboxProcess = try {
